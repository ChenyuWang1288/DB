...
public:
  ...
  using KeyTypeName=KeyType;

  using ValueTypeName=ValueType;

  using KeyComparatorName=KeyComparator;
  ...
  dberr_t ScanKey(const Row &key, std::vector<RowId> &result, int &position, page_id_t &leaf_page_id, Transaction *txn);
