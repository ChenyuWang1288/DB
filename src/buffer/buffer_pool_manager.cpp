...
Page *BufferPoolManager::FetchPage(page_id_t page_id) {
  ...
  if (page_table_.find(page_id) != page_table_.end()) {  //内存中如果能找到这个page直接返回就可以
    ...
    replacer_->Pin(page_table_(page_id));  // pin一下(replacer已经被定义）
    latch_.unlock();
    return p;
  } else {//如果在内存中没找到就需要在内存中寻找一个位置存放读取的page
    frame_id_t frame_id_;//定义一个变量存位置
    frame_id_t *frame_id;//定义一个指针进行参数传递
    if (free_list_.size() != 0) {//如果有自由区
      ...
      bool n = replacer_->Victim(frame_id);//将参数传过去，在LRU中会将替换的id传过来
    ...
    pages_[frame_id_].data_ = newdata;//向内存中写入新数据
...
Page *BufferPoolManager::NewPage(page_id_t &page_id) {
  ...
  page_table_.erase(R->page_id_);
  page_table_.insert(newpage, frame_id_);  //找到了p要写入的位置：frame_id并且更新相关的位置信息。
  pages_[frame_id_].data_ = null;           //向内存中写入新数据
...
bool BufferPoolManager::DeletePage(page_id_t page_id) {
  ...
  free_list_.push_back(page_table_[page_id]);//将内存里的这一块内容放入到自由区中便于后续的使用
  latch_.unlock();
  return true;
...
bool BufferPoolManager::UnpinPage(page_id_t page_id, bool is_dirty) {
  ...
  if (is_dirty == 1) pages_[page_table_[page_id]].is_dirty_ = true;//如果是脏页面，修改值
...
bool BufferPoolManager::FlushPage(page_id_t page_id) {
  if (page_table_.find(page_id) == page_table_.end() || page_id == INVALID_PAGE_ID) return false;
